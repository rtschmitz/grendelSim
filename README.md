...
The upper tunnel tracker consists of four tracker stations. The wall-adjacent station begins 1 mm from the tunnel wall, preserving the previous wall clearance. The other stations have edge-to-edge clear distances of 12 cm, 24 cm, and 36 cm from the inward face of the wall station. Each station contains a 1.5 cm phi-segmented scintillator plane, a 1 mm air gap, and a 1.5 cm longitudinally segmented scintillator plane, for a 3.1 cm envelope. Beneath each inner station, a pair of unsegmented 2 cm-thick veto volumes follows the same lower-wall endpoints as the original wall veto and extends down to the floor boundary; no additional floor layers are created. Both orientations have a nominal 1 cm channel pitch; the longitudinal planes use native Geant4 replicas for efficient navigation.

Sensitive-hit copy numbers preserve the original stations: `1000 + phiBin` and `10000 + zBin` for the wall station, and `2000 + phiBin` and `20000 + zBin` for the 24 cm station. The new 12 cm and 36 cm stations use layer prefixes 3 and 5 respectively: phi bases 3000 and 5000, and longitudinal bases 30000 and 50000. The original floor and lower-wall veto pieces remain copy numbers 0--2. The two wall extensions at 12 cm, 24 cm, and 36 cm use copy-number pairs 100--101, 200--201, and 300--301 respectively; there are no corresponding added floor volumes.
