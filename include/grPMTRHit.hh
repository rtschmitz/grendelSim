...
class grPMTRHit: public TObject
{
public:
  ...
  grPMTRHit(const grPMTRHit &right);

  const grPMTRHit& operator=(const grPMTRHit &right);
  Int_t operator==(const grPMTRHit &right) const;
...
//  bool GetScintToPMT(){return scintToPMT;}
  ...
  static bool compareHits(grPMTRHit* const &a, grPMTRHit* const &b);
...
//  Double_t totalEDep_eV;
  Int_t    parentID;
//  bool scintToPMT;
  ClassDef(grPMTRHit,8);

};
typedef std::vector<grPMTRHit*> grPMTRHitVector;
