...
public:
  ...
  grPMTHit();
  ~grPMTHit();
  grPMTHit(const grPMTHit &right);

  const grPMTHit& operator=(const grPMTHit &right);
  G4int operator==(const grPMTHit &right) const;
...
typedef G4THitsCollection<grPMTHit> grPMTHitsCollection;
typedef std::vector<grPMTHit*> grPMTHitVector;

extern G4Allocator<grPMTHit> grPMTHitAllocator;

inline void* grPMTHit::operator new(size_t){
  void *aHit;
  aHit = (void *) grPMTHitAllocator.MallocSingle();
  return aHit;
}

inline void grPMTHit::operator delete(void *aHit){
  grPMTHitAllocator.FreeSingle((grPMTHit*) aHit);
}
