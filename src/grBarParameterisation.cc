...
//
//
/// \file grBarParameterisation.cc
/// \brief Implementation of the grBarParameterisation class
...
grBarParameterisation::grBarParameterisation(
        G4int    nBarX,
        G4int    nBarY,
...
void grBarParameterisation::ComputeTransformation
...
void grBarParameterisation::ComputeDimensions
(G4Box& grBar, const G4int copyNo, const G4VPhysicalVolume*) const
{
  // Note: copyNo will start with zero! and for now, we don't change our geom size for each copy
  //grBar.SetXHalfLength(fHalfWidth);
  //grBar.SetYHalfLength(fHalfWidth);
  //grBar.SetZHalfLength(fHalfLength);
}
