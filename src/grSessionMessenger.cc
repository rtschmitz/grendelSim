...
grSessionMessenger::grSessionMessenger(grSession* Sess) :
  mySession(Sess) {
#ifdef DEBUG_MSG
  G4cout << "enter grSessionMessenger::grSessionMessenger" << G4endl;
#endif

  char logExtension[6] = ".log";
  G4String defaultFileName = grGenFileName::genFileName(logExtension);
...
#ifdef DEBUG_MSG
  G4cout << "leaving grSessionMessenger::grSessionMessenger" << G4endl;
#endif
}

grSessionMessenger::~grSessionMessenger() {
  delete dirLogging;
  delete cmdLogFileName;
}

void grSessionMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
#ifdef DEBUG_MSG
  G4cout << "enter grSessionMessenger::SetNewValue" << G4endl;
#endif
