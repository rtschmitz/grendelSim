...
#include "grPrimaryGeneratorActionMessenger.hh"
...
grPrimaryGeneratorAction::grPrimaryGeneratorAction(grDetectorConstruction* myDC) :
    ...
    MsgPrefix = "grPrimaryGeneratorAction> ";
    myMessenger = new grPrimaryGeneratorActionMessenger(this);
    this->InitPGA();
...
void grPrimaryGeneratorAction::UseGeantinos(G4bool myGeantinoStatus){
...
void grPrimaryGeneratorAction::InitPGA() {
...
void grPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent) {
  ChangeSeeds();
  //StoreSeeds();
...
/*void grPrimaryGeneratorAction::StoreSeeds(){ //TODO store seeds vie recorder
...
void grPrimaryGeneratorAction::ChangeSeeds(){
  //Restore seeds
  if(!UseDefaultSeeds){
    G4cout << "/grendelSim> - BeginOfEventAction: Change random number generator seeds from :" << G4endl;
...
void grPrimaryGeneratorAction::SetSeeds(const unsigned long seed0,
    const unsigned long seed1, const unsigned long seed2) {
  Seeds.push_back(1878463799);//ID to identify the used random number generator, here it is the RanecuEngine (see grendelSim.cc:28)
  ...
  G4cout << "grendelSim/RunAction> SetSeeds" << G4endl;
