...
grRunAction::grRunAction(grHistoManager* histo) :
  histoManager(histo){

  sprintf(filename,"Sim_%d");
  DataFileNamePrefix = G4String(filename);

  runMessenger = new grRunActionMessenger(this);
 ...
 */
void grRunAction::BeginOfRunAction(const G4Run* aRun) {
...
/**
 * @brief  Passed a pointer to the finished run to the grAnalysisManager.
 * @parameter  aRun  Pointer to the current run, which just finished.
 */
void grRunAction::EndOfRunAction(const G4Run* aRun) {
