...
//
//
/// \file grPMTParameterisation.cc
/// \brief Implementation of the grPMTParameterisation class
...
grPMTParameterisation::grPMTParameterisation(
        G4int    nBarX,
        G4int    nBarY,
...
void grPMTParameterisation::ComputeTransformation
...
void grPMTParameterisation::ComputeDimensions
(G4Tubs& grPMT, const G4int copyNo, const G4VPhysicalVolume*) const
{
  // Note: copyNo will start with zero! and for now, we don't change our geom size for each copy
  //grPMT.SetInnerRadius(0);
  //grPMT.SetOuterRadius(fRadius);
  //grPMT.SetStartPhiAngle(0*deg);
  //grPMT.SetDeltaPhiAngle(360*deg);
  //grPMT.SetZHalfLength(fHalfLength);
}
