...
output_script="resubmit_flattening_jobs.sh"
  ...
  if [ -d "$mass_dir" ]; then
    ...
    for charge_dir in "$mass_dir"/*; do
      if [ -d "$charge_dir" ]; then
        ...
        flattened_file="${charge_dir}/flattened_output.root"

        # Check if flattened_output.root is missing or outdated
        if [ ! -f "$flattened_file" ]; then
          echo "flattened_output.root is missing for mass: $mass, charge: $charge"
	  #echo "bsub /net/cms17/cms17r0/schmitz/python310/bin/python3.10 flatlightwithphoton.py "$charge_dir"/ MilliQan" >> "$output_script"
        else
          # Use find to compare the file modification date
          file_date=$(stat -c %y "$flattened_file" | cut -d' ' -f1)
          if [[ "$file_date" < "$date_cutoff" ]]; then
            echo "flattened_output.root is outdated for mass: $mass, charge: $charge (file date: $file_date)"
	    echo "bsub /net/cms17/cms17r0/schmitz/python310/bin/python3.10 flatlightwithphoton.py "$charge_dir"/ MilliQan" >> "$output_script"
          fi
        fi
      fi
