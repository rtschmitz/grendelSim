...
for mass_dir in "$G4output_dir"/*; do
  if [ -d "$mass_dir" ]; then
    ...
    mass=$(basename "$mass_dir")
	  ...
	  output_dir="${G4output_dir}/${mass}/${charge}/"

          echo "Running flatlightwithphoton.py for mass: $mass, charge: $charge"
	  bsub /net/cms17/cms17r0/schmitz/python310/bin/python3.10 flatlightwithphoton.py "$output_dir" MilliQan
        fi
    done
    fi
  #fi
